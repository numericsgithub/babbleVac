...
rem FOR %%A IN (%*) DO sox %%A "converted/%%~nxA" rate -v 62500
...
rem command.
rem 
rem %%A --norm=-1 -e unsigned-integer -b 8 -r 31250 -c 1 -t raw "converted/%%~nxA"

cd %~dp0
mkdir converted
FOR %%A IN (%*) DO sox %%A --norm=-1 -e unsigned-integer -b 8 -r 62500 -c 1 -t raw "converted/%%~nxA"
pause
